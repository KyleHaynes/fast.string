...
trim_clean <- short_unique
trim_padded <- paste0(" \t", short_unique, "\r\n")
...
# Paired base/fast trim cases cover both the no-change path and source slices.
add_case(
    "trimws-base-clean",
    "base::trimws",
    "trim comparison",
    corpus_metadata("clean short strings", short_n, string_bytes = 18L,
                    match_density = 0, unique_fraction = 1),
    function(thread) function() {
        base::trimws(trim_clean)
    },
    threaded = FALSE
)
add_case(
    ...
    corpus_metadata("clean short strings", short_n, string_bytes = 18L,
                    match_density = 0, unique_fraction = 1),
    function(thread) function() {
        fast.string::ftrimws(trim_clean)
    },
    threaded = FALSE
)
add_case(
    ...
    "base::trimws",
    "trim comparison",
    corpus_metadata("padded short strings", short_n, string_bytes = 22L,
                    match_density = 1, unique_fraction = 1),
    function(thread) function() {
        base::trimws(trim_padded)
    },
    threaded = FALSE
)
add_case(
    ...
    corpus_metadata("padded short strings", short_n, string_bytes = 22L,
                    match_density = 1, unique_fraction = 1),
    function(thread) function() {
        fast.string::ftrimws(trim_padded)
    },
    threaded = FALSE
)
...
# Explicitly exercise both preparation rejection branches: unsupported q and
# a 1 x N shape whose non-empty side has no useful q-gram reuse.
add_case(
    "qgram-matrix-q9-rejected",
    "jaccard_matrix",
    "q-gram rejected path",
    corpus_metadata(
        "balanced repeated q=9",
        matrix_side,
        matrix_side,
        string_bytes = 11L,
        q = 9L,
...
add_case(
    "qgram-matrix-low-reuse-rejected",
    "jaccard_matrix",
    "q-gram rejected path",
    corpus_metadata(
        "1 x N with empty left input",
        1L,
        matrix_n,
        string_bytes = 13L,
        q = 2L,
...
add_case(
    "fuzz-partial-ratio-full-process",
    "fuzz_partial_ratio",
    "fuzzy normalisation",
    corpus_metadata("punctuation and whitespace", pair_n, string_bytes = 20L,
                    unique_fraction = 3 / pair_n),
...
add_case(
    "jaro-token-default-strip",
    "jaro_winkler_tokens",
    "token matching",
    corpus_metadata("default punctuation stripping", pair_n, string_bytes = 20L,
                    unique_fraction = 3 / pair_n),
    function(thread) function() {
        fast.string::jaro_winkler_tokens(
            fuzzy_a, fuzzy_b, nthreads = thread
        )
    },
    all_threads = TRUE
)
...
results <- list()
for (spec in specs) {
    case_threads <- if (!isTRUE(spec$threaded)) {
        NA_integer_
    } else if (isTRUE(spec$all_threads)) {
        thread_grid
    } else {
        unique(c(1L, max(thread_grid)))
    }
    for (thread in case_threads) {
        thread_label <- if (is.na(thread)) "n/a" else sprintf("%2d", thread)
        cat(sprintf("  %-39s threads=%3s ... ", spec$id, thread_label))
