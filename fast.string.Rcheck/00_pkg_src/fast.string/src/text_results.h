...
inline bool store_source_slice(const StringView& source,
                               const char* value,
                               std::size_t value_length,
                               TextResult& result) {
    if (value_length > source.size) return false;
    if (value_length == 0) {
        result.kind = TextResultKind::source_slice;
        result.offset = 0;
        result.length = 0;
        return true;
    }
    ...
    if (found == source.data + source.size) return false;
    result.kind = TextResultKind::source_slice;
    result.offset = static_cast<std::size_t>(found - source.data);
...
inline bool store_source_slice(const StringView& source,
                               ...
                               TextResult& result) {
    return store_source_slice(
        source, value.data(), value.size(), result
    );
}
...
inline void store_text_output(std::size_t row,
    ...
    if (allow_source_slice &&
        store_source_slice(source, data, length, results[row])) {
        return;
    }
    store_changed_text(
