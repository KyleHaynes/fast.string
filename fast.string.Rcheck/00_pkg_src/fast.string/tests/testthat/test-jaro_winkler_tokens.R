...
test_that("tokenisation handles legacy whitespace normalisation without materialising it", {
    a <- c(
        "  Kyle\tJohn  Haynes ", "\tSMITH\n", "A\r\nB",
        "", " ", NA_character_
    )
    b <- c(
        "John Kylie\r\nHaynes", "SMYTH", "AB",
    ...
    expect_identical(
        fast.string::jaro_winkler_tokens(
            a, b, strip = NULL, nthreads = 1
        ),
        fast.string::jaro_winkler_tokens(
            normalise_ws(a), normalise_ws(b), strip = NULL, nthreads = 1
        )
    )
})

test_that("token whitespace remains deliberately narrower than isspace", {
    expect_equal(
        fast.string::jaro_winkler_tokens(
            "A\tB", "AB", strip = NULL, nthreads = 1
        ),
        1
    )
    expect_lt(
        fast.string::jaro_winkler_tokens(
            "A\vB", "AB", strip = NULL, nthreads = 1
        ),
        1
    )
    expect_lt(
        fast.string::jaro_winkler_tokens(
            "A\fB", "AB", strip = NULL, nthreads = 1
        ),
        1
    )
})
